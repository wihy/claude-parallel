...
_CHAT_INPUT: Optional["ChatInputSession"] = None
...
# ── Claude 调用 (薄封装到 src.claude_client) ─────────────
...
def call_claude(
    ...
    委托给 src.claude_client.call_claude，此处保留 Rich UI 日志。
    """
    # Rich-colored log wrapper
    model_chain = parse_model_chain(
        ",".join(planner_models) if planner_models and isinstance(planner_models, list) and not isinstance(planner_models[0], str) else ""
    ) if planner_models is None else planner_models

    for model_idx, model_name in enumerate(model_chain or [""]):
        if model_name:
            console.print(f"  [cyan][planner][/cyan] 使用模型: [bold]{model_name}[/] ({model_idx + 1}/{len(model_chain or [''])})")
        else:
            console.print(f"  [cyan][planner][/cyan] 使用默认模型 ({model_idx + 1}/{len(model_chain or [''])})")

    success, output, error = _call_claude_impl(
        prompt=prompt,
        system=system,
        budget=planner_budget,
        retries=planner_retries,
        model_chain=planner_models,
    )

    if not success:
        console.print(f"  [dim]\\[debug] 失败: {error[:200]}[/]")
